package com.ignek.background.listener;
...
import com.ignek.background.job.api.BackgroundJobApi;
...
public class BackgroundJobModelListener extends BaseModelListener<DLFileEntry> {
	private static final Log _log = LogFactoryUtil.getLog(BackgroundJobModelListener.class);
	...
	public void onAfterUpdate(DLFileEntry originalModel, DLFileEntry fileEntry) throws ModelListenerException {
		...
		try {
			...
			BackgroundTaskLocalServiceUtil.addBackgroundTask(userId, groupId, "CSV Import Task",
					BackgroundJobApi.class.getName(), taskContextMap, new ServiceContext());
